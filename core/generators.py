...
TERRACE_GEOMETRY = {
    1: {"slope_degree": 12.5, "bund_height_m": 0.45}, 
    2: {"slope_degree": 14.0, "bund_height_m": 0.50}, 
    3: {"slope_degree": 15.5, "bund_height_m": 0.65}  
}
...
def generate_tanadasynth(num_records):
    ...
    coupled thermodynamics, and realistic Gaussian sensor noise.
    """
    dataset = []
    ...
    for i in range(num_records):
        ...
        # 2. Resolving the Decoupled Physics: Thermodynamic calculation
        base_temp = random.uniform(20.0, 35.0)
        base_velocity = 1450.0 + (4.21 * base_temp) - (0.037 * (base_temp ** 2))
        ...
        # Injecting Gaussian Sensor Noise
        moisture = random.gauss(base_moisture, 2.0) 
        velocity = random.gauss(base_velocity, 8.0) 
        ...
        record = {
            ...
            "slope_degree": TERRACE_GEOMETRY[elevation_tier]["slope_degree"],
            "bund_height_m": TERRACE_GEOMETRY[elevation_tier]["bund_height_m"],
            "weather_forecast": forecast,
            "moisture_pct": round(max(0, moisture), 2),
